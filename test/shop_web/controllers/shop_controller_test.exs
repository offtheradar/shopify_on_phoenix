defmodule ShopWeb.ShopControllerTest do
  ...
  describe "create shop" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, shop_path(conn, :create), shop: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == shop_path(conn, :show, id)

      conn = get conn, shop_path(conn, :show, id)
  ...
  describe "update shop" do
    ...
    test "redirects when data is valid", %{conn: conn, shop: shop} do
      conn = put conn, shop_path(conn, :update, shop), shop: @update_attrs
  ...
  describe "delete shop" do
    ...
    test "deletes chosen shop", %{conn: conn, shop: shop} do
      ...
      assert_error_sent 404, fn ->
        get conn, shop_path(conn, :show, shop)
      end
    end
  end
